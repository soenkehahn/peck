...
withDb ::
  ...
  (Db a -> IO b) ->
  IO b
withDb context path action = do
  migrate context (Proxy :: Proxy a) path
  ...
  when exists $ do
    forM_ migrations $ \(migration :: Migration a) -> do
      ...
      case maybeContents of
        Nothing -> return ()
        Just elements -> do
          log context ("performing migration: " ++ name migration)
